...
module.exports = function admin(ctx) {
  ...
  // ---- Google sign-in for designers (hosted deployments) ----
  async function handleAuth(req, res, url) {
    const redirectUri = `${CONFIG.mainOrigin}/auth/google/callback`;
    if (url.pathname === '/auth/google') return ctx.G.start(req, res, redirectUri);
    if (url.pathname !== '/auth/google/callback') throw httpError(404, 'Not found');
    const { email, name } = await ctx.G.finish(req, url, redirectUri);
