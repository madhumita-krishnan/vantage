...
function randomId(bytes = 9) { return crypto.randomBytes(bytes).toString('base64url'); }
function randomToken(bytes = 32) { return crypto.randomBytes(bytes).toString('base64url'); }
function randomKeyHex() { return crypto.randomBytes(32).toString('hex'); }
...
function verifyPasscode(pass, rec) {
  if (!rec) return false;
  const hash = crypto.scryptSync(String(pass), rec.salt, 32).toString('hex');
...
function chunkedWriter(blob, file) {
  ...
  function flush(all) {
    while (pendingLen >= CHUNK || (all && pendingLen > 0)) {
      ...
      pending = rest.length ? [rest] : []; pendingLen = rest.length;
      if (!all && pendingLen < CHUNK) break;
    }
  }
  return {
    write(c) { pending.push(c); pendingLen += c.length; if (pendingLen >= CHUNK) flush(false); },
    end() { flush(true); fs.closeSync(fd); closed = true; },
    abort() { try { fs.closeSync(fd); } catch { /* ignore */ } try { fs.unlinkSync(file); } catch { /* ignore */ } closed = true; },
  };
}
