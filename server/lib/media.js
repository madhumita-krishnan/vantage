...
module.exports = function media(ctx) {
  ...
  function storeIntro(req, id, limit) {
    return new Promise((resolve, reject) => {
      ...
      req.on('data', (c) => { if (failed) return; size += c.length; if (size > limit) { failed = true; w.abort(); req.destroy(); reject(H.httpError(413, `Media larger than ${Math.round(limit / 1048576)} MB`)); } else w.write(c); });
      req.on('end', () => { if (!failed) { w.end(); resolve(size); } });
      req.on('error', (e) => { if (!failed) { failed = true; w.abort(); reject(e); } });
    });
  }
  ...
  function range(req, res, size, maxChunk) {
    ...
    if (m && size > 0) {
      if (m[1]) start = +m[1];
      if (m[2]) end = Math.min(+m[2], size - 1); else if (!m[1]) start = Math.max(0, size - (+m[2] || 0));
      if (start >= size || start > end) { res.writeHead(416, { 'Content-Range': `bytes */${size}` }); res.end(); return null; }
  ...
  async function stream(req, res, r, size, mime, pieces, extra = {}) {
    const headers = { ...H.baseHeaders(req), 'Content-Type': mime, 'Accept-Ranges': 'bytes', 'Content-Length': Math.max(0, r.end - r.start + 1), ...extra };
  ...
  async function streamRecording(req, res, share, session) {
    ...
    for (let i = 0; i < rec.segments; i++) {
      const f = path.join(dir, `${i}.bin`); let st; try { st = fs.statSync(f); } catch { continue; }
      const len = st.size - (blob.enabled ? 32 : 0); segs.push({ f, start: total, len }); total += len;
    }
    ...
    const pieces = (function* () {
      for (const s of segs) {
        ...
        yield blob.decode(fs.readFileSync(s.f)).subarray(Math.max(0, r.start - s.start), Math.min(s.len, r.end - s.start + 1));
      }
    })();
    await stream(req, res, r, total, rec.mime, pieces, { 'Content-Disposition': `inline; filename="voice-${rec.viewerId}.webm"` });
  }
  function appendRecording(share, session, viewerId, mime, seq, buf) {
    share.recordings = share.recordings || {};
    const rec = share.recordings[session] || (share.recordings[session] = { viewerId, mime, size: 0, segments: 0, startedAt: Date.now(), updatedAt: Date.now() });
    if (Object.values(share.recordings).reduce((a, x) => a + x.size, 0) + buf.length > CONFIG.maxMediaBytes) throw H.httpError(413, 'Recording storage limit reached');
    const dir = recordingDir(share.id, session);
    fs.mkdirSync(dir, { recursive: true, mode: 0o700 });
    fs.writeFileSync(path.join(dir, `${seq}.bin`), blob.encode(buf), { mode: 0o600 });
    rec.segments = Math.max(rec.segments, seq + 1); rec.size += buf.length; rec.updatedAt = Date.now();
    return rec;
  }
  ...
  const removeSubtitle = (share, lang) => { try { fs.unlinkSync(subtitleFile(share.id, lang)); } catch { /* ignore */ } };
  const removeRecording = (share, session) => fs.rmSync(recordingDir(share.id, session), { recursive: true, force: true });
  return { storeIntro, streamIntro, streamRecording, appendRecording, removeRecording, writeSubtitle, readSubtitle, removeSubtitle };
};
