...
let win = null, lockedOrigin = null, pending = null;
...
const PASTE_PAGE = 'data:text/html;charset=utf-8,' + encodeURIComponent(`<!doctype html><meta charset="utf-8"><title>Prototype Vault</title>
...
function open(target) {
  if (!win) {
    win = new BrowserWindow({ width: 1280, height: 860, title: 'Prototype Vault', autoHideMenuBar: true, webPreferences: { sandbox: true, contextIsolation: true, nodeIntegration: false } });
    win.setContentProtection(true);
    win.on('closed', () => { win = null; lockedOrigin = null; });
    const allowed = (url) => { try { const o = new URL(url).origin; if (!lockedOrigin && /^https?:/.test(url)) lockedOrigin = o; return o === lockedOrigin; } catch { return false; } };
    win.webContents.on('will-navigate', (e, url) => { if (!allowed(url)) { e.preventDefault(); if (/^https?:/.test(url)) shell.openExternal(url); } });
    win.webContents.setWindowOpenHandler(({ url }) => { if (/^https?:/.test(url) && !allowed(url)) shell.openExternal(url); return { action: 'deny' }; });
  }
  if (target) { lockedOrigin = new URL(target).origin; win.loadURL(target); } else win.loadURL(PASTE_PAGE);
  win.show(); win.focus();
}
...
if (!app.requestSingleInstanceLock()) app.quit();
  ...
  app.on('open-url', (e, url) => { e.preventDefault(); pending = linkFrom(url); if (app.isReady()) open(pending); }); // macOS
